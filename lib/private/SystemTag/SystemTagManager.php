...
class SystemTagManager implements ISystemTagManager {
	 ...
	 * {@inheritdoc}
	 */
	public function getAllTags($visibilityFilter = null, $nameSearchPattern = null, IUser $user = null) {
		$tags = [];
		...
		while ($row = $result->fetch()) {
			...
			/**
			 * If the tag is static tag and not in the whitelisted group just skip it,
			 * because the user has no access to assign/unassign it.
			 */
			if (($user !== null) && $this->isStaticTag($tag) &&
				!$this->canUserUseStaticTagInGroup($tag, $user)) {
				continue;
			}
			$tags[$row['id']] = $tag;
		}
	 ...
	 * @return bool true if tag is static tag, else false
	 * @since 10.0.11
	 */
	private function isStaticTag(ISystemTag $tag) {
		$visibility = $tag->isUserVisible();
		$assignable = $tag->isUserAssignable();
		$editable = $tag->isUserEditable();
		return  ($visibility === true && $editable === false && $assignable === true) ? true : false;
	}
}
