...
class SystemTagManagerTest extends TestCase {
	...
	public function provideStaticTag() {
		return [
			[true],
			[false]
		];
	...
	/**
	 * @dataProvider provideStaticTag
	 * @param $isStaticTag
	 */
	public function testIsStaticTag($isStaticTag) {
		if ($isStaticTag === true) {
			$tag = $this->tagManager->createTag('tag1', true, false, false);
		} else {
			$tag = $this->tagManager->createTag('tag1', true, false, true);
		}

		$result = $this->invokePrivate($this->tagManager, 'isStaticTag', [$tag]);
		$this->assertEquals($result, $isStaticTag);
	}
}
